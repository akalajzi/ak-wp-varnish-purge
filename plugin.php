<?php
/*
Plugin Name: akalajzi Varnish Purge
Description: Create varnish purging requests on all reverse proxies after posts are modified
Version: 0.1.0
Author: akalajzi

    Varnish servers are behind load balancer accessible directly through different ports.

    Fork of a fork of a fork... Licences/mentions bellow.
...
class VarnishPurger {
    ...
    function varnish_rightnow() {
        if (current_user_can('activate_plugins')) {
            ...
            $text = $button;
            $dashForce = 'Ovo gori govno vjerojatno ne radi, al mos ga probat stisnit ako triba. Za osvjezit naslovnicu stisni ovi botun doli.';
            $dashForceUrl = wp_nonce_url(admin_url('?vhp_flush_index'), 'varnish-http-purge');
            $dashForceButton = sprintf( __('<p class="button"><a href="%1$s"><strong>Osvjezi naslovnicu</strong></a></p>', 'varnish-http-purge' ), $dashForceUrl );
            $dashForceText = $dashForce . '<br />' . $dashForceButton;
            echo "<p class='varnish-rightnow'>$text</p>\n";
            echo "<p class='varnish-rightnow'>$dashForceText</p>\n";
        }
    }
    ...
    public function executePurge() {
      ...
      if (isset($_GET['vhp_flush_index']) && current_user_can('manage_options')) {
    ...
    protected function purgeUrl($url) {
        ...
        // create requests on both servers through lb
        $urlWp1 = $p['scheme'].'://'.$p['host'].':81'.$p['path'];
        $urlWp2 = $p['scheme'].'://'.$p['host'].':82'.$p['path'];

        wp_remote_get($urlWp1, array('headers' => array('Cache-Control' => 'no-cache', 'Jebagati' => 'prazni-varnish')));
        wp_remote_get($urlWp2, array('headers' => array('Cache-Control' => 'no-cache', 'Jebagati' => 'prazni-varnish')));
        // debug: wp-content/debug.log
        //error_log("Did wp_remote_get " . $urlWp1 . ' | '. $urlWp2);
